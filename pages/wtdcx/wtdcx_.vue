...
export default {
	data() {
		return {
			...
			//url: 'http://192.168.1.148:8080/#/pages/index/index',
	...
	onLoad(options) {
		// 从首页传参
		//console.log(JSON.parse(options.query).filterText);
		...
		if (options.query) {
			...
			this.q = q;
			this.url = this.url0 + '?token=' + encodeURIComponent(this.token) + '&q=' + q + '&t=1';
			...
			*/
			...
			this.url = this.url0 + '?token=' + encodeURIComponent(this.token) + '&t=1';
	},
	methods: {
		getMessage(event) {
			uni.showModal({
	...
	onNavigationBarSearchInputConfirmed(e) {
		this.url = this.url0 + '?q=' + e.text + '&t=1';
		//console.log(this.url);
		// #ifdef APP-PLUS
