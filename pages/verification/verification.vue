				...
				<view class="cu-dialog" @tap.stop="">
					...
					<view class="cu-bar bg-white justify-end">
						<view class="action">
							...
							<button class="cu-btn bg-green margin-left" @tap="doQuery()">确定</button>
						</view>
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		downCallback(mescroll) {
			//console.log('下拉刷新1:' + mescroll);
			//console.log('下拉刷新2:' + meScroll);
			//下拉刷新
			if (mescroll) {
				meScroll = mescroll;
			}
		...
		async getListDataFromNet(pageNum, pageSize, successCallback, errorCallback) {
			if (this.qjmcNames.length == 0) return;
			this.pagerequest.skipCount = (pageNum - 1) * pageSize;
			this.pagerequest.WTDH = this.wtdh;
			this.pagerequest.FilterText = this.radio1 + ',' + this.query;
	...
	onShow() {
		...
		this.lstlength = 0;
		if (this.qjmcNames.length > 0) this.downCallback(meScroll);
	}
};
</script>
